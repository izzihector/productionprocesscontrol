...
class ProductSubscription(models.Model):
    ...
    sale_subscription_line_id = fields.Many2one('sale.subscription.line', string='Subscription line', required=True,
                                                ondelete='cascade', index=True, copy=False)
    product_id = fields.Many2one('product.product', string='Product')
    subscription_id = fields.Many2one('sale.subscription', string='Subscription',
                                      related='sale_subscription_line_id.analytic_account_id')
