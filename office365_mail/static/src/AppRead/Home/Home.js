...
(function () {
    ...
    Office.initialize = function (reason) {
        $(document).ready(function () {
            ...
            Office.context.mailbox.addHandlerAsync(Office.EventType.ItemChanged, doc.itemChanged);
    ...
    /* --- Navigation --- */
    doc.toSettingsButton = $('#to_settings')
    doc.goBackButton = $('#go_back')
    ...
    /* --- Update UI after item change --- */
    doc.itemChanged = function (eventArgs) {
        // Update UI based on the new current item
        doc.UpdateTaskPaneUI(Office.context.mailbox.item);
    }

    doc.UpdateTaskPaneUI = function (item)
    {
    ...
    home.onAttachedItemClick = function () {
        ...
        var url = new URL(app.odooUrl + '/redirect/' + model + '/' + item);
        url.host = '.' + url.host;
    ...
    home.loadModelItems = function (model) {
        odoo.getModelItemsAsync({
            ...
            send_mail: _currentItem.sender.emailAddress,
            send_name: _currentItem.sender.displayName,
            success: function (data) {
                doc.searchBox.val('');
                doc.searchBox.focus();
    ...
    home.showConnectionError = function () {
        doc.loadingField.fadeOut(150, function () {
            doc.errorField.fadeTo(150, 1);
        });
    }
    ...
    home.showAttachingError = function () {
        doc.attachingLoadingField.fadeOut(150, function () {
            doc.attachingErrorField.css({ opacity: 0, display: 'inline-block' }).animate({ opacity: 1 }, 150);
    ...
    home.showCreatingError = function () {
        doc.createLoadingField.fadeOut(150, function () {
            doc.createErrorField.css({ opacity: 0, display: 'inline-block' }).animate({ opacity: 1 }, 150);
            doc.createButton.prop('disabled', false);
        });
    }

    /* --- Show Fields --- */
    home.showGoBack = function () {
        var time = 150;

        doc.toSettingsButton.fadeOut(time);

        setTimeout(function () {
            doc.goBackButton.fadeIn(time);
        }, time);
    }

    home.showGoSettings= function () {
        var time = 150;

        doc.goBackButton.fadeOut(time);

        setTimeout(function () {
            doc.toSettingsButton.fadeIn(time);
        }, time);
    }
    ...
    home.showInfo = function (source) {
        ...
        function show() {
            home.hideFields(function () {
                ...
                if (reset) reset();
                doc.infoField.fadeIn(150);
            });
        }

        if (newItem) {
            loadingField.fadeOut(150, function () {
                successField.fadeIn(150);
                setTimeout(function () {
                    show();
                }, 1000);
            });
    ...
    home.attachToOdooWithAttachments = function (params) {
        ...
        params.itemId = Office.context.mailbox.item.itemId;
    ...
    home.createMail = function (params) {
        ...
        var current_datetime = _currentItem.dateTimeCreated
        var prefix = '<i>Original email from '
            + _currentItem.sender.displayName + ' &lt;' + _currentItem.sender.emailAddress
            + ' to '
            + _currentItem.to.map(function(to) { return to.displayName + " <" + to.emailAddress + ">"}).join("; ")
            + ' at '
            + toJSONLocal(_currentItem.dateTimeCreated).replace("T", " ")
            + '<br/>CC: '
            + _currentItem.cc.map(function(cc) { return cc.displayName + " <" + cc.emailAddress + ">"}).join("; ")
            + '</i><br/><hr style="border-color: #ccc; margin: 5px;"/><br/>'
            + 'Subject: ' + params.subject + '<br/><br/>'

        var suffix = '<br/><hr style="border-color: #ccc; margin: 5px;"/><br/>Created with the <i><strong>Outlook Odoo Connector</strong></i> by <i><strong>Somko</strong></i><br/><br/>'

        params.body = prefix + params.body.replace('</body>', '') + suffix
        params.send_date = _currentItem.dateTimeCreated.toISOString().substring(0, 19)
        params.sender_email = _currentItem.sender.emailAddress
        params.sender_name = _currentItem.sender.displayName
    ...
    home.addAttachedItem = function (item) {
        ...
        _customProperties.saveAsync(function (e) {
            console.log('Saved Custom Properties');
        });
    }
